 ...
 * @return {number} A position of the found element (starting at 0)
 ...
 *
 * @param {string} [encoding] An encoding of the string. Defaults to `utf8`.
 * @return {string} A string constructed from the shifted bytes or empty string
 ...
 * @param {string} [encoding] An encoding of the string. Defaults to `utf8`.
 * @return {string} A string constructed from the read bytes or empty string if
...
{
  return ieee754.readIEEE754(
    this.readBytes(offset, 4), 0, !littleEndian, 23, 4
  );
};
...
{
  return ieee754.readIEEE754(
    this.readBytes(offset, 8), 0, !littleEndian, 52, 8
  );
};
